...
fn bench_parse_semantic_module(c: &mut Criterion) {
    let source = include_str!("../test_data/semantic_module.prsm");
...
fn bench_parse_multi_syntax(c: &mut Criterion) {
            let c_like = include_str!("../test_data/c_like_syntax.prsm");
        let python_like = include_str!("../test_data/python_like_syntax.prsm");
        let rust_like = include_str!("../test_data/rust_like_syntax.prsm");
...
fn bench_cohesion_analysis(c: &mut Criterion) {
    let source = include_str!("../test_data/large_module.prsm");
...
fn bench_documentation_validation(c: &mut Criterion) {
    let source = include_str!("../test_data/documented_module.prsm");
    let program = parse_program(source).unwrap();
...
fn bench_effect_analysis(c: &mut Criterion) {
    let source = include_str!("../test_data/effectful_module.prsm");
    let program = parse_program(source).unwrap();
...
fn test_real_world_payment_module() {
    let source = include_str!("../test_data/payment_processing.prsm");
...
fn test_real_world_user_management() {
    let source = include_str!("../test_data/user_management.prsm");
...
**Note**: This grammar specification has been superseded by **[PLT-000: Conversational Grammar](PLT-000.md)**, which provides the canonical grammar specification for human-thinking code. The grammar below represents the traditional formal specification that maps to the conversational patterns defined in PLT-000.

For the current canonical grammar specification, see **[PLT-000: Conversational Grammar](PLT-000.md)**.
