...
```prism
// Effect inference example using PLT-000 conversational patterns
what happens when processing user data?
given userData: UserData
{
    // Compiler infers: Database.Query + Cryptography.Encryption + IO.Network
    validate user data using database
    encrypt validated data using cryptography
    send encrypted result over network
}
gives ProcessedData or Error

// Explicit effect annotation
what happens when processing user data?
given userData: UserData
expecting database query, cryptography encryption, network IO effects
{
    // Implementation with explicit effect tracking
    validate and encrypt user data
    send result securely
}
gives ProcessedData or Error
```
...
capability RevocableCapability<T> {
    ...
    what happens when using capability?
    given operation: T -> R
    {
        when capability is revoked
        then return CapabilityError::Revoked
        
        when capability is available
        then execute operation with capability
        
        when capability not available
        then return CapabilityError::NotAvailable
    }
    gives R or CapabilityError
    
    what happens when revoking capability?
    {
        mark capability as revoked
        clear inner capability
    }
    ensures capability cannot be used again
}
```
...
// Endorsement for integrity using PLT-000 conversational patterns
what happens when endorsing data integrity?
given data: Labeled<T> and new_level: IntegrityLevel and authority: EndorsementAuthority
{
    // Verify the authority can endorse to this level
    when authority cannot endorse to new level
    then return SecurityError::InsufficientAuthority
    
    when authority is valid
    then create labeled data with:
        same data content
        new integrity level
        same other label properties
}
gives Labeled<T> or SecurityError
```
...
capability PromptExecution {
    ...
    what happens when executing secure request?
    given request: SecureRequest
    expecting Network.ExternalAPI effects
    {
        // Validate request for injection attempts
        validate request for security issues
        
        // Apply input filters
        apply content filters to validated request
        
        // Execute with safety constraints
        call external API with filtered request and constraints
        
        // Post-process response
        sanitize response for safe consumption
    }
    gives SafeResponse or APIError
}
```
...
module PaymentProcessor {
    ...
    section trusted {
        ...
        expecting database query, cryptography encryption effects
        {
            // Core payment logic - no external dependencies
            validate payment request
            encrypt sensitive payment data
            query database for account verification
            process payment securely
        }
        gives PaymentResult or Error
    }
...
module SecureWebService {
    ...
    section interface {
        ...
        expecting database query, cryptography hashing effects
        {
            create user credentials with:
                username: request.username
                password_hash: request.password_hash
                security label: Secret, Authenticated, for Authentication
            
            authenticate user with credentials
            
            // Safe to log authentication attempt (no sensitive data)
            log authentication attempt with:
                username: request.username
                timestamp: now()
        ...
        gives AuthResponse or AuthError
...
module AICodeAnalyzer {
    ...
    section interface {
        ...
        function batch_analyze(directory: Path) -> Result<BatchAnalysisReport, AnalysisError>
            ...
            let files = filesystem.list_files(directory, "*.prsm")?;
            let mut reports = [];
